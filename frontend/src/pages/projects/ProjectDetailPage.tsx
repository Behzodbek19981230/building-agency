...
import { MapPin, Clock, DollarSign, Star, CheckCircle, Loader2, ArrowLeft } from 'lucide-react';
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {project.images?.length > 0 && (
            <div className="grid grid-cols-2 gap-2 rounded-xl overflow-hidden">
              {project.images.slice(0, 4).map((img: any, i: number) => (
                <img key={img.id} src={img.url} alt="" className={clsx('w-full object-cover', i === 0 ? 'col-span-2 h-64' : 'h-36')} />
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { icon: Clock, label: 'Shoshilinchlik', value: project.urgency },
              ...
              <div className="divide-y">
                {project.bids.map((bid: any) => (
                  <div key={bid.id} className="p-4 flex items-center gap-4">
                    <img src={bid.worker.avatar || `https://ui-avatars.com/api/?name=${bid.worker.firstName}`} alt="" className="w-10 h-10 rounded-full" />
        ...
        <div className="space-y-4">
          <div className="card p-5">
            <div className="flex items-center gap-3 mb-4">
              <img
                src={project.client.avatar || `https://ui-avatars.com/api/?name=${project.client.firstName}`}
            ...
            {user?.role === 'WORKER' && project.status === 'OPEN' && (
              <Link to={`/projects/${project.id}#bid`} className="btn-primary w-full text-center py-2.5">
                Taklif berish
              </Link>
            )}
          </div>
